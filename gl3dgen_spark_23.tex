...
lens systems in 3D, opposed to conventional methods limited to 2D. \\[0pt]
...
\label{sec:org1feac3d}

\subsection{State of research}
\label{sec:orgca6f640}
\label{sec:research}
...
\item The modelling tools, as well as the trained deep learning models,
...
\label{sec:orgc8f1246}

The project is organized in a work package of sequential tasks
(WP[1-5]) as follows:

\begin{itemize}[leftmargin=*, noitemsep]
\item WP1: \emph{gathering publicly available base datasets} (duration: < 1 month)
...
\end{enumerate}
\item WP2: \emph{building the training dataset} (duration: 1-2 months)
\begin{itemize}[leftmargin=*]
...
Section~\ref{sec:methods} could easily be implemented to
run on the available GPU cluster available to us.
\end{itemize}
\item WP3: \emph{testing and training deep learning architecture} (duration: 4-5 months)
\begin{itemize}[leftmargin=*]
\item this is the most compute-intensive task and will be run in
parallel on our GPU science cluster of 8 NVIDIA A100 GPUs and 24
V100 GPUs. The network architectures mentioned in
Section~\ref{sec:methods} are typically designed for
processing 2D or voxel data, so significant adjustments and
novelty will be necessary to adapt them to our specific data
requirements.
\end{itemize}
\item WP4: \emph{integration into a lens modelling framework} (duration: 2-3 months)
\begin{itemize}[leftmargin=*]
\item strategies from studies mentioned in WP2 may be adapted for this
purpose, or a new strategy could be explored.
\end{itemize}
\item WP5: \emph{benchmarking and real-world testing} (duration: 1 month)
\begin{enumerate}[leftmargin=*]
